...
**ゲストモード。**「Continue as guest」を押すと Supabase の匿名セッションが開始し、
サインアップなしでアプリを試せます。ゲストも実体は匿名の「ユーザー」なので、リクエストは
従来どおり認証され、クエリも従来どおりそのユーザーにスコープされます。つまりセキュリティ
モデルは変わりません。異なるのは次の点です。

| | ゲスト | アカウント |
|---|---|---|
| 1 時間あたりのシミュレーション | 2 回 | 10 回 |
| 1 時間あたりのインタビュー送信 | 20 回 | 100 回 |
| 実行・インタビュー・パネル・エクスポート | 可 | 可 |
| 公開共有リンク | 不可 | 可 |
| 別の端末への引き継ぎ | 不可(セッションはこのブラウザ内のみ) | 可 |

サイドバーからメールアドレスとパスワードを登録すると、匿名ユーザーがそのままアカウントに
昇格します。ゲストのときに実行した内容はすべて新しいアカウントに引き継がれます。
...
- 利用回数の制限はユーザー単位とサービス全体の両方で `usage_events` テーブルに基づいて適用され、
  ...
  API 全体を保護します。
- ゲストかどうかは Supabase がユーザーに付与する `is_anonymous` クレームで判定します。
  そのためブラウザ側が「正規アカウントである」と偽って制限を外すことはできません。
...
1 つの Node プロセスが API とビルド済みクライアントの両方を配信するため、**プロセスが常駐する
ホスト**が適しています(Render、Railway、Fly.io、Cloud Run など)。設計上、次の 2 点がそれに
依存しています。`/api/simulate` は AI の処理が終わるまでレスポンスをストリーミングし続けること、
そしてブラウザが切断されても実行結果の保存まで完了させること。静的ホスティングや、実行時間の
上限が短いサーバーレス環境では、これらが途中で打ち切られます。

[`render.yaml`](render.yaml) をブループリントとして用意してあります。Render で
**New → Blueprint** を選び、このリポジトリを指定して、プロンプトで 4 つの秘密情報を入力すれば
完了です(`TRUST_PROXY` と Node のバージョンは設定済み)。手動で作る場合は **Web Service** を
作成し、Build Command に `npm ci && npm run build`、Start Command に `npm start` を設定して、
5 つの環境変数を登録します。Render はビルド時にも環境変数を渡すため、`VITE_*` の値が
クライアントバンドルに正しく埋め込まれます。

公開できたら、Supabase の **Site URL** をデプロイ先の URL に設定してください(確認メールの
リンク先になります)。なお Render の無料インスタンスは約 15 分アクセスがないとスリープし、
復帰に 1 分ほどかかります。人がいきなりクリックする場所にリンクを置く前に、把握しておくと
よい点です。
